...
import com.example.whoisthat.databinding.FragmentPlayer1ScreenBinding
...
class Player1ScreenFragment : Fragment(),HasCustomTitle,CharacterActionListener,HasCustomAction{

    private var options: Options=Options.DEFAULT

    private lateinit var adapter1: CharacterAdapter
    private lateinit var binding: FragmentPlayer1ScreenBinding
    ...
    override fun onCreateView(
        ...
        binding= FragmentPlayer1ScreenBinding.inflate(inflater,container,false)
        ...
        binding.player1Button.isClickable=false
        binding.player1Button.setBackgroundColor(resources.getColor(R.color.white))
        binding.player2Button.setBackgroundColor(resources.getColor(R.color.purple_200))
        binding.player2Button.setOnClickListener{navigator().showPLayer2Screen(options)}
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance(options:Options):Player1ScreenFragment{
            val arguments = Bundle()
            arguments.putParcelable(OPTIONS_PARAM, options)
            val fragment = Player1ScreenFragment()
    ...
    private fun setUpRecyclerView(){
        ...
        adapter1.characters=CharacterService(options).getCharacters1()
...
//    }
    ...
    override fun getTitleRes(): Int=R.string.player1_screen_text
    ...
    override fun changeChosen(view: View,imageView: ImageView) {
        ...
        if(!character.isChosen){
            ...
            options.charIndexesChosen1.add(options.charIndexesChosen1.size,character.ind)
        }
        else{
            character.isChosen=false
            character.resource=character.passiveResource
            imageView.setImageResource(character.resource)
            options.charIndexesChosen1.remove(character.ind)
        }
